...
public class NullableIssue {
  ...
  * @throws IOException if the JSON Object is invalid with respect to NullableIssue
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!NullableIssue.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!NullableIssue.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `NullableIssue` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArraylabels.size(); i++) {
        IssueLabelsInner.validateJsonObject(jsonArraylabels.get(i).getAsJsonObject());
      };
      ...
      if (jsonObj.get("closed_by") != null && !jsonObj.get("closed_by").isJsonNull()) {
        NullableSimpleUser.validateJsonObject(jsonObj.getAsJsonObject("closed_by"));
      }

...
public class CombinedCommitStatus {
  ...
  * @throws IOException if the JSON Object is invalid with respect to CombinedCommitStatus
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!CombinedCommitStatus.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CombinedCommitStatus.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CombinedCommitStatus` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArraystatuses.size(); i++) {
        SimpleCommitStatus.validateJsonObject(jsonArraystatuses.get(i).getAsJsonObject());
      };

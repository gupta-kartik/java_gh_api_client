...
public class BranchRestrictionPolicyAppsInnerOwner {
  ...
  **/
  ...
  @Override
  public int hashCode() {
    return Objects.hash(login, id, nodeId, url, reposUrl, eventsUrl, hooksUrl, issuesUrl, membersUrl, publicMembersUrl, avatarUrl, description, gravatarId, htmlUrl, followersUrl, followingUrl, gistsUrl, starredUrl, subscriptionsUrl, organizationsUrl, receivedEventsUrl, type, siteAdmin);
  }
  ...
  * @throws IOException if the JSON Object is invalid with respect to BranchRestrictionPolicyAppsInnerOwner
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!BranchRestrictionPolicyAppsInnerOwner.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!BranchRestrictionPolicyAppsInnerOwner.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BranchRestrictionPolicyAppsInnerOwner` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

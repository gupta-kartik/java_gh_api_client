...
public class TeamDiscussion {
  ...
  * @throws IOException if the JSON Object is invalid with respect to TeamDiscussion
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!TeamDiscussion.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!TeamDiscussion.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TeamDiscussion` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

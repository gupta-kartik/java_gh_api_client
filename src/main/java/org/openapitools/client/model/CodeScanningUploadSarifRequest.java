...
public class CodeScanningUploadSarifRequest {
  ...
  * @throws IOException if the JSON Object is invalid with respect to CodeScanningUploadSarifRequest
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!CodeScanningUploadSarifRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CodeScanningUploadSarifRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CodeScanningUploadSarifRequest` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

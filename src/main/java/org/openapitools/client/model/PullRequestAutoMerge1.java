...
public class PullRequestAutoMerge1 {
  ...
  * @throws IOException if the JSON Object is invalid with respect to PullRequestAutoMerge1
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!PullRequestAutoMerge1.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PullRequestAutoMerge1 is not found in the empty JSON string", PullRequestAutoMerge1.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PullRequestAutoMerge1.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PullRequestAutoMerge1` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : PullRequestAutoMerge1.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PullRequestAutoMerge1.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PullRequestAutoMerge1' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PullRequestAutoMerge1> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PullRequestAutoMerge1.class));

       return (TypeAdapter<T>) new TypeAdapter<PullRequestAutoMerge1>() {
           @Override
           public void write(JsonWriter out, PullRequestAutoMerge1 value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PullRequestAutoMerge1 read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of PullRequestAutoMerge1 given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of PullRequestAutoMerge1
  * @throws IOException if the JSON string is invalid with respect to PullRequestAutoMerge1
  */
  public static PullRequestAutoMerge1 fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PullRequestAutoMerge1.class);
  }

 /**
  * Convert an instance of PullRequestAutoMerge1 to an JSON string
  *
  * @return JSON string
  */

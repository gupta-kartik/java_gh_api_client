...
public class CodeScanningAlert {
  ...
  * @throws IOException if the JSON Object is invalid with respect to CodeScanningAlert
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!CodeScanningAlert.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CodeScanningAlert is not found in the empty JSON string", CodeScanningAlert.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CodeScanningAlert.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CodeScanningAlert` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : CodeScanningAlert.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CodeScanningAlert.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CodeScanningAlert' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CodeScanningAlert> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CodeScanningAlert.class));

       return (TypeAdapter<T>) new TypeAdapter<CodeScanningAlert>() {
           @Override
           public void write(JsonWriter out, CodeScanningAlert value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CodeScanningAlert read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of CodeScanningAlert given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of CodeScanningAlert
  * @throws IOException if the JSON string is invalid with respect to CodeScanningAlert
  */
  public static CodeScanningAlert fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CodeScanningAlert.class);
  }

 /**
  * Convert an instance of CodeScanningAlert to an JSON string
  *
  * @return JSON string
  */

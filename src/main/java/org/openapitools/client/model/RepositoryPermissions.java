...
public class RepositoryPermissions {
  ...
  * @throws IOException if the JSON Object is invalid with respect to RepositoryPermissions
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!RepositoryPermissions.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in RepositoryPermissions is not found in the empty JSON string", RepositoryPermissions.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!RepositoryPermissions.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `RepositoryPermissions` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : RepositoryPermissions.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!RepositoryPermissions.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'RepositoryPermissions' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<RepositoryPermissions> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(RepositoryPermissions.class));

       return (TypeAdapter<T>) new TypeAdapter<RepositoryPermissions>() {
           @Override
           public void write(JsonWriter out, RepositoryPermissions value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public RepositoryPermissions read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of RepositoryPermissions given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of RepositoryPermissions
  * @throws IOException if the JSON string is invalid with respect to RepositoryPermissions
  */
  public static RepositoryPermissions fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, RepositoryPermissions.class);
  }

 /**
  * Convert an instance of RepositoryPermissions to an JSON string
  *
  * @return JSON string
  */

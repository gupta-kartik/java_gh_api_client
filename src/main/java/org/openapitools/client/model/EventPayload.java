...
public class EventPayload {
  ...
  private List<EventPayloadPagesInner> pages;
  ...
  * @throws IOException if the JSON Object is invalid with respect to EventPayload
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!EventPayload.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!EventPayload.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `EventPayload` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("pages") != null && !jsonObj.get("pages").isJsonNull()) {
        ...
        if (jsonArraypages != null) {
          ...
          for (int i = 0; i < jsonArraypages.size(); i++) {
            EventPayloadPagesInner.validateJsonObject(jsonArraypages.get(i).getAsJsonObject());
          };
        }
      }
  }
